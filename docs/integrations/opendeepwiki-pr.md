...
If the user's repo isn't yet in the registry, log a friendly one-liner pointing at `npx @looptech-ai/understand-quickly-cli add` or the wizard — don't fail the parent run.
...
- [ ] With the token set but the repo unregistered, the log message points at `npx @looptech-ai/understand-quickly-cli add`; no failure.
